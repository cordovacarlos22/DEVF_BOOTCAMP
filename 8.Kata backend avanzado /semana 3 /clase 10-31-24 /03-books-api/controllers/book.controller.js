...
const getAllBooks = async (req, res) => {
  try {
    ...
    if (!books) return res.status(404).json({ message: 'Not Books Found' })
...
const getBookById = async (req, res) => {
  ...
  try {
    ...
    const books = await Book.find({ _id: req.params.bookId, isActive: true }).populate('authors', 'firstName lastName bio birthDate -_id')
    if (!books) return res.status(404).json({ message: 'Not Book Found' })
    res.status(200).json(books)
  } catch (error) {
    res.status(400).json({ message: error.message })
  }
...
const updateBookById = async (req, res) => {
  ...
  try {
    const book = await Book.findByIdAndUpdate(req.params.bookId, req.body, { new: true }).populate('authors', 'firstName lastName bio birthDate -_id')
